...
class M_Proforma_Detail extends Model
{
  ...
  // --
  public function create_Proforma($billingData, $products)
  {
    try {
      $this->pdo->beginTransaction();

      $this->setSeriesAndCorrelative($billingData);
      $billingId = $this->insertProforma($billingData);
      $this->insertProformaDetails($billingId, $products, $billingData['fecha_emision']);
  ...
  private function insertProforma($billingData)
  {
    ...
    $stmt = $this->pdo->prepare($sql);
    $stmt->execute($this->prepareProformaData($billingData));
    return $this->pdo->lastInsertId();
  }

  private function prepareProformaData($billingData)
  {
  ...
  private function insertProformadetails($billingId, $products, $saleDate)
  {
  ...
  {
    $billingpersale = $this->getProforma($billingId);
    $billingpersale_details = $this->getProformadetails($billingId);
  ...
  private function getProforma($billingId)
  {
    $sqlProforma = 'SELECT 
        b.id, 
    ...
    $stmt = $this->pdo->prepare($sqlProforma);
    $stmt->bindParam(':id_sale', $billingId, PDO::PARAM_INT);
    $stmt->execute();
    return $stmt->fetch(PDO::FETCH_ASSOC);
  }

  private function getProformadetails($billingId)
  {
  ...
  private function createDET($proforma, $proforma_details, $companyInfo)
  {
    $detalles = [];

    foreach ($proforma_details as $detail) {
      $codigo = $detail['code'];
    ...
    $path = __DIR__ . "/../../files/DATA/";
    $nameDET =  $companyInfo['ruc'] . "-" . $proforma['code'] . "-" . $proforma['series'] . "-" . $proforma['correlative'] . ".DET";
